...
class SmsReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        try {
            if (bundle != null){
                /**
                bundle dg key "pdus" sudah merupakan standar yang diguanakn oleh system
                 */

                val pdusObj = bundle.get("pdus") as Array<*>
                for (aPdusObj in pdusObj){
                    val currentMessage = getIncommingMessage(aPdusObj as Any, bundle)
    ...
    private fun getIncommingMessage(aObject : Any, bundle: Bundle) : SmsMessage{
